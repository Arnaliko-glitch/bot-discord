...
export default async function UsersPage({ params }: { params: { id: string } }) {
  ...
  return (
    ...
    >
      <h1 className="mb-2 text-2xl font-bold">Utilisateurs</h1>
      <p className="mb-8 text-gray-400">Classement XP du serveur</p>
      ...
      <div className="card overflow-hidden p-0">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {users.map((u, i) => {
              ...
              return (
                <tr key={u.id} className="border-b border-[var(--border)]/50 hover:bg-[var(--card-hover)]">
                  <td className="px-4 py-3 text-gray-500">{i + 1}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <img src={userAvatarUrl(u.discordUserId, u.avatar, 32)} alt="" className="h-8 w-8 rounded-full" />
                      <span className="font-medium">{u.username}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3">{u.level}</td>
                  <td className="px-4 py-3">{u.xp.toLocaleString('fr-FR')}</td>
        ...
        {users.length === 0 && <p className="p-8 text-center text-gray-400">Aucun utilisateur avec de l&apos;XP</p>}
      </div>
    </DashboardShell>
  );
}
